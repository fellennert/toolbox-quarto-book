...
Chapter 2, "Brief R Recap," will briefly introduce RStudio Projects, Quarto, tidy data and `tidyr`, `dplyr`, `ggplot`, functions, loops, and `purrr`. These techniques are elementary for the things that come next.

Chapter 3, "`stringr` and RegExes," deals with string manipulation using, you guessed it, `stringr` and Regexes.

Chapters 4 and 5 introduce the reader to the basics of `rvest`, HTML, and CSS selectors and how these can be used to acquire data from the web. Moreover, I introduce the `httr` package and explain how you can use it to make requests to APIs.

Chapter 5, "Text Preprocessing and Basic Analyses," touches upon the basics of bringing text into a numeric format that lends itself to quantitative analyses. Moreover, it introduces feature weighting (i.e., determining which tokens matter more than others) as well as dictionary-based analyses.

Chapter 6, "Text Classification with the Bag of Words," deals with the classification of text in both supervised and unsupervised ways.

Chapter 7, "Beyond BoW," finally introduces new text analysis techniques that are based on distributional representations of words, commonly referred to as word embeddings.
